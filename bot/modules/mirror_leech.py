...
class Mirror(TaskListener):
    ...
    async def new_event(self):
        ...
        if file_ is not None:
            msg_caption = await remove_unwanted(f'{reply_to.caption if reply_to.caption else ""}')
            f_name = self.name if self.name else msg_caption
            await TelegramDownloadHelper(self).add_download(
                reply_to, f"{path}/", f_name, session
            )
